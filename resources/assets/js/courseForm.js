    ...
    function buildHolesForm(holeCount) {
        ...
        for (var i = 0; holeCount > i; i++) {
            currentHoleGroup.append(
                "<div class='hole'>"+
                    "<input type='text' name='hole_number' maxLength='2' value='"+(i+1)+"' class='col-xs-2 num-only' required>"+
                    "<div class='col-xs-1 text-center seperator'>-</div>"+
                    "<input type='text' name='hole_length' maxLength='3' class='col-xs-3 num-only' required>"+
                "</div>"
            )
        }
    }
